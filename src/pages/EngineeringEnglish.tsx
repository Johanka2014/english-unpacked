...
const EngineeringEnglish = () => {
  return (
    <div className="min-h-screen bg-background">
      <SEO
        title="Cambridge English for Engineering | English Unpacked"
        description="Cambridge English for Engineering: 10 units of professional English for engineers — functions, materials, design, safety, monitoring and more."
      />
      <Navigation />

      <section className="relative py-24 flex items-center justify-center text-center bg-gradient-to-br from-slate-800 to-slate-600">
        <div className="container mx-auto px-4">
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">
            Cambridge English for Engineering
          </h1>
          <p className="text-xl text-white/90 max-w-2xl mx-auto">
            10 units of professional English for engineers — from describing technology to pushing the boundaries
          </p>
        </div>
      </section>
      ...
      <main className="container mx-auto px-4 py-12">
        <div className="mb-8">
          <Link to="/members/activities?tab=engineering">
            <Button variant="ghost" className="gap-2">
              <ArrowLeft className="h-4 w-4" /> Back to Engineering Activities
            </Button>
          </Link>
        </div>

        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {engineeringUnits.map((unit, i) => {
            const gradient = gradients[i % gradients.length];
            return (
              <Link key={unit.id} to={`/engineering-english/${unit.id}`}>
