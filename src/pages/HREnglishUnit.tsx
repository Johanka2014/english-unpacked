...
import { ArrowLeft, BookOpen, Headphones, PenLine, MessageCircle, Languages, Lock } from 'lucide-react';
...
const HREnglishUnit = () => {
  ...
  if (!unit) return <Navigate to="/hr-english" replace />;

  return (
    <div className="min-h-screen bg-background">
      <SEO
        title={`Unit ${unit.number}: ${unit.title} | Oxford English for HR`}
      ...
      <main className="container mx-auto px-4 py-12">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {unit.skills.map((skill) => {
            ...
            if (hasContent) {
              return (
                <Link key={skill.id} to={`/hr-english/${unit.id}/${skill.id}`}>
                  {card}
                </Link>
              );
            }
