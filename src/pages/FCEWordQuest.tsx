...
} from "@/data/fceWordsData";
import { useFceProgress, BADGES, level, xpForLevel, xpToNext } from "@/store/fceProgress";
...
const FCEWordQuest = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEO
        title="FCE Word Quest — Cambridge B2 Vocabulary Game"
        description="Gamified vocabulary trainer for the Cambridge FCE (B2) exam — flashcards, quizzes, spelling and daily challenges."
      />
      <Navigation />
      <PetStyles />
...
function HomeView({ setView }: { setView: (v: View, extra?: Record<string, string>) => void }) {
  const { xp, streak, correctTotal } = useFceProgress();
  const lvl = level(xp);
  ...
  return (
    <div>
      <header className="mb-6">
        <p className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">FCE Vocab</p>
...
function PlayIndexView({ setView }: { setView: (v: View, extra?: Record<string, string>) => void }) {
  const tricky = useFceProgress((s) => s.trickyWordIds);
  const modes = [
...
function PlayRound({ mode, topic, tricky, onExit }: { mode: GameMode; topic?: Topic; tricky: boolean; onExit: () => void }) {
  const trickyIds = useFceProgress((s) => s.trickyWordIds);
  const recordAnswer = useFceProgress((s) => s.recordAnswer);
  const finishRound = useFceProgress((s) => s.finishRound);
  const soundOn = useFceProgress((s) => s.soundOn);
...
function WordsView({ setView }: { setView: (v: View, extra?: Record<string, string>) => void }) {
  ...
  return (
    <div>
      ...
      <p className="mb-4 text-sm text-muted-foreground">{ALL_WORDS.length} FCE vocabulary words</p>
...
function ProgressView() {
  ...
  return (
    <div>
      ...
      <p className="mt-6 text-center text-xs text-muted-foreground">
        {ALL_WORDS.length} FCE words available · Cambridge B2 level
      </p>
    </div>
  );
}
