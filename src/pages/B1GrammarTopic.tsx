...
const B1GrammarTopic = () => {
  ...
  const [activeTab, setActiveTab] = useState<'theory' | 'exercises' | 'exam' | 'practice' | 'wordwall' | 'holiday' | 'cambridge' | 'modal-master' | 'so-such' | 'compound-adjectives' | null>(null);
  ...
  const hasContent = !!(mod.theory || mod.exercises || mod.examPractice?.people || mod.examPractice?.readingPart1 || mod.tenseMaster || mod.wordwall || mod.holidayLesson || mod.cambridgeLesson || mod.modalMasteryLesson || mod.soSuchLesson || mod.compoundAdjectivesLesson);
  ...
  const tiles = [
    ...
    { key: 'so-such' as const, label: 'So & Such', icon: Wand2, available: !!mod.soSuchLesson, color: 'from-rose-600 to-pink-700' },
            ...
            <div className="max-w-5xl mx-auto">
              ...
              {(() => {
                ...
                return <>
                  ...
                  {tab === 'so-such' && mod.soSuchLesson && <SoSuchLesson />}
